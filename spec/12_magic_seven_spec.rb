...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the previous step is 15 and random_number is 10' do
      subject(:random_number) { described_class.new(10) }

      it 'returns 5' do
        previous_step = 15
        result = random_number.subtract_random_number(previous_step)
  ...
  describe '#play' do
    context 'when the random number is 22' do
      subject(:random_number) { described_class.new(22) }
    ...
    context 'when the random number is 2' do
      subject(:random_number) { described_class.new(2) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = random_number.play
        expect(result).to eq(7)
    ...
    context 'when the random number is 78' do
      subject(:random_number) { described_class.new(2) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = random_number.play
